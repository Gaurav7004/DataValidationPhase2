...
class PopUpProgressBar(QWidget):
    ...
    def __init__(self):
        ...
        self.setGeometry(700, 500, 650, 10)
...
"background-color: rgb(255, 255, 255);")
    ...
    def onSelectState(self, index):
        try:
            ...
            self.msg.setStyleSheet("QLabel { margin-right: 15px ; font-size: 18px; font-family: Arial;} QPushButton {background-color:lightgreen; font-family: Arial; font-size:20px;} ")
            # Set the main message
            self.msg.setText("By default everthing has been selected automatically \n\n डिफ़ॉल्ट रूप से सब कुछ स्वचालित रूप से चुना")
            # Set the title of the window
            self.msg.setWindowTitle(" ")
            # Display the message box
            self.msg.show()
    ...
    def filterdataState(self, final_list):
        global df
        df = df[df['col_3'].isin(final_list)]
        return df
    ...
    def onSelectDistrict(self, index):
        try:
            ...
            self.msg.setStyleSheet("QLabel { margin-right: 15px ; font-size: 18px; font-family: Arial;} QPushButton {background-color:lightgreen; font-family: Arial; font-size:20px;} ")
            # Set the main message
            self.msg.setText("By default everthing has been selected automatically \n\n डिफ़ॉल्ट रूप से सब कुछ स्वचालित रूप से चुना जा चुका है")
            # Set the title of the window
            self.msg.setWindowTitle(" ")
            # Display the message box
            self.msg.show()
    ...
    def filterdataDistrict(self, final_list):
        global df
        df = df[df['col_5'].isin(final_list)]
        return df
    ...
    def filterdataSubDistrict(self, final_list):
        global df
        df = df[df['col_7'].isin(final_list)]
        return df
    ...
    def filterdataBlock(self, final_list):
        global df
        df = df[df['col_8'].isin(final_list)]
        return df
    ...
    def onSelectHealthBlock(self, index):
        try : 
            ...
            # app_2 = QtWidgets.QApplication(sys.argv)
            form = ChecklistDialog('Health Block', item, checked=True)
            if form.exec_() == QtWidgets.QDialog.Accepted:
                final_list = [str(s) for s in form.choices]
    ...
    def filterdataHealthBlock(self, final_list):
        global df
        df = df[df['col_9'].isin(final_list)]
        return df
    ...
    def filterdataRuralUrban(self, final_list):
        global df
        df = df[df['col_18'].isin(final_list)]
        return df
    ...
    def filterdataOwnership(self, final_list):
        global df
        df = df[df['col_19'].isin(final_list)]
        return df
